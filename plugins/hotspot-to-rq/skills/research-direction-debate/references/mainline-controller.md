...
The JSON above is the controller-produced logical directive. In a schema-1.4
`CLAUDE` session, the Orchestrator first persists the exact dispatch input,
then deterministically adds only `transport_path` and `transport_sha256` to
each logical dispatch before controller validation and state commit. Those
transport fields are not generated by the controller. A `CODEX` directive
keeps the exact logical shape above because its bytes are bound by the separate
ready batch manifest.
...
existing schema-1.3 or schema-1.4 control history. `POST_USER_GATE` must immediately consume
...
committed controller dispatch. Do not upgrade schema 1.1, 1.2, or 1.3 history
in place; start a new schema-1.4 session, fix its `transport_profile`, and
reference the legacy artifacts from its evidence pack.
...
dispatch's complete transport input to
`control-inputs/dispatches/<packet id>.json` inside the session directory.
For a schema-1.4 `CLAUDE` session, publish an immutable owner-read-only JSON
object with `schema_version: claude-dispatch-input-1` and the exact
`envelope`, `role_instructions`, `inline_payload`,
`allowed_artifact_paths`, and `search_budget` sent to the role. Re-invocation
and cross-session resume rebuild the call from those bytes — never from the
conversation transcript or a runtime-scoped workflow run id, which do not
survive the session. Before running `validate_controller_decision.py`, enrich
each Claude logical dispatch with the canonical `transport_path`
(`control-inputs/dispatches/<packet id>.json`) and the lowercase
`transport_sha256` of those immutable bytes. Commit those two fields unchanged
in `transition_log`; both validators compare the digest and path. A Search
extension must be present verbatim in that committed immutable input.

On Codex, follow `codex-port.md`: after validating the controller directive,
build every immutable final role packet and pass
`validate_codex_dispatch_batch.py` before committing the transition. Initial
dispatch and transport retry first verify the committed batch manifest, then
read that final packet; neither may rebuild it from a draft or current source
artifact. Codex builders and batch gates require schema 1.4 with
`transport_profile: CODEX`; Claude-bound sessions cannot enter this path.
